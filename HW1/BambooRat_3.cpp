...
void partition(vector<Point>&points,vector<Point>&right,vector<Point>&left);
    ...
    for(int i=0;i<n;i++)
    {
        ...
        for(int j=0;j<n;j++)
        {
            ...
            if((lB==lR)&&(rR==rB))
            {
                for(int j=0;j<lR;j++)
                {
                    right.push_back(points[rightR[j]]);
                    left.push_back(points[leftR[j]]);
                }
                for(int j=0;j<rR;j++)
                {
                    right.push_back(points[rightB[j]]);
                    left.push_back(points[leftB[j]]);
                }
                points.clear();
                points.push_back(A);
                points.push_back(B);
